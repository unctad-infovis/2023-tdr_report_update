    ...
    Highcharts.chart(`chartIdx${idx}`, {
      ...
      tooltip: {
        ...
        formatter() {
          ...
          const rows = [];
          rows.push(values.map((point, i) => `<div><span class="tooltip_label">${(point[0]) ? `${point[0]}: ` : ''}</span><span class="tooltip_value">${roundNr(point[1], (i === 2) ? 1 : 0).toLocaleString('en-US')} ${suffix[i]}</span></div>`).join(''));
          // eslint-disable-next-line react/no-this-in-sfc
